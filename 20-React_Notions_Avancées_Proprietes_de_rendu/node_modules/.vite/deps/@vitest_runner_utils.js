...
var serialize$1 = (element, config, indentation, depth, refs, printer2) => ++depth > config.maxDepth ? printElementAsLeaf(getType(element), config) : printElement(
  getType(element),
  printProps(
...
var serialize = (object, config, indentation, depth, refs, printer2) => ++depth > config.maxDepth ? printElementAsLeaf(object.type, config) : printElement(
  object.type,
...
function createFileTask(filepath, root, projectName, pool) {
  const path2 = relative(root, filepath);
  const file = {
    id: generateHash(`${path2}${projectName || ""}`),
    name: path2,
    type: "suite",
    mode: "run",
    filepath,
...
@vitest/pretty-format/dist/index.js:
   ...
   *)
*/
//# sourceMappingURL=@vitest_runner_utils.js.map
